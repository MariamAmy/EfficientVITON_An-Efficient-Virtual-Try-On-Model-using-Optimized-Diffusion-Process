...
class VITONHDDataset(Dataset):
    def __init__(
        ...
        im_names = []
        c_names = []
        with open(opj(self.drd, f"inference_pairs.txt"), "r") as f:
    ...
    def __getitem__(self, idx):
        ...
        if self.transform_size is None and self.transform_color is None:
            agn = imread(
                opj(self.drd, "inference", "agnostic-v3.2", self.im_names[idx]), 
                self.img_H, 
                self.img_W
            )
            agn_mask = imread(
                opj(self.drd, "inference", "agnostic-mask", self.im_names[idx].replace(".jpg", "_mask.png")), 
                self.img_H, 
                self.img_W, 
                is_mask=True, 
            ...
            cloth = imread(
                opj(self.drd, "inference", "cloth", self.c_names[self.pair_key][idx]), 
                self.img_H, 
                self.img_W
            )
            cloth_mask = imread(
                opj(self.drd, "inference", "cloth-mask", self.c_names[self.pair_key][idx]), 
                self.img_H, 
                self.img_W, 
                is_mask=True, 
            ...
            gt_cloth_warped_mask = imread(
                opj(self.drd, "inference", "gt_cloth_warped_mask", self.im_names[idx]), 
                self.img_H, 
                self.img_W, 
                is_mask=True
            ) if not self.is_test else np.zeros_like(agn_mask)

            image = imread(opj(self.drd, "inference", "image", self.im_names[idx]), self.img_H, self.img_W)
            image_densepose = imread(opj(self.drd, "inference", "image-densepose", self.im_names[idx]), self.img_H, self.img_W)

        else:
            agn = imread_for_albu(opj(self.drd, "inference", "agnostic-v3.2", self.im_names[idx]))
            agn_mask = imread_for_albu(opj(self.drd, "inference", "agnostic-mask", self.im_names[idx].replace(".jpg", "_mask.png")), is_mask=True)
            cloth = imread_for_albu(opj(self.drd, "inference", "cloth", self.c_names[self.pair_key][idx]))
            cloth_mask = imread_for_albu(opj(self.drd, "inference", "cloth-mask", self.c_names[self.pair_key][idx]), is_mask=True, cloth_mask_check=True)
            ...
            image = imread_for_albu(opj(self.drd, "inference", "image", self.im_names[idx]))
            image_densepose = imread_for_albu(opj(self.drd, "inference", "image-densepose", self.im_names[idx]))
