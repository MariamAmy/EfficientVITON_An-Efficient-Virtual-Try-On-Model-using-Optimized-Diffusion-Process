...
class ControlLDM(LatentDiffusion):
    ...
    def forward(self, x, c, *args, **kwargs):
        if not self.use_pbe_weight:
            # Instead of random t, sample from self.ddim_timesteps
            t = random.choice(self.ddim_timesteps)  # Sample from the non-uniform schedule
            t = torch.full((x.shape[0],), t, device=self.device, dtype=torch.long) #convert to proper shape.
            ...
            if self.model.conditioning_key is not None:
                ...
                if not isinstance(c, dict): # if not dict make it dict.
                    c = [c]
                    c = {'c_crossattn': c}
                    
                if "c_crossattn" in c.keys():
                    cond = c["c_crossattn"]
                    if len(cond.shape) == 3:
                        cond = cond[:, None, :]
                    if "c_concat" in c.keys():
                        ...
                        if isinstance(c_concat, list):
                            c_concat = c_concat[0]
                        c = {"c_concat": [c_concat], "c_crossattn": [cond]}
                    else:
                        c = {"c_crossattn":[cond]}
                        
                # Modify cond based on timestep t
                if self.shorten_cond_schedule:
                    tc = extract_into_tensor(self.cond_ids, t, x.shape)
                    if "c_crossattn" in c.keys():
                        c["c_crossattn"][0] = self.q_sample(x_start=c["c_crossattn"][0], t=tc, noise=torch.randn_like(c["c_crossattn"][0]))
                    else:
                        c = self.q_sample(x_start=c, t=tc, noise=torch.randn_like(c))
                

            return self.p_losses(x, c, t, *args, **kwargs)  # Pass t to p_losses
